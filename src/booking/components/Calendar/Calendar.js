...
const STATION_NAMES = {
  1: "Station 01 — Body Wash",
  2: "Station 02 — Body Wash",
  3: "Station 03 — Body Wash + Interior",
  4: "Station 04 — Full Service",
};

const Calendar = ({ stationId }) => {
  ...
  const fetchAvailableSlots = async () => {
    try {
      const response = await fetch(
        `/Backend/api.php?action=get_calendar&station_id=${stationId}&month=${currentDate.getMonth() + 1}&year=${currentDate.getFullYear()}`
      );
  ...
  const handleDayClick = (day) => {
    if (day && day >= new Date(new Date().setHours(0, 0, 0, 0))) {
      ...
      navigate(`/booking-station/${stationId}/timeslot/${formattedDate}`);
    }
  };
    ...
    <div className="calendar">
      <div className="header">
        ...
        <div className="current-month">
          {STATION_NAMES[stationId] || `Station ${stationId}`} —{" "}
