...
const Calendar = () => {
  ...
  const fetchAvailableSlots = async () => {
    try {
      const response = await fetch(
        `http://localhost/Backend/api.php?action=get_calendar&month=${currentDate.getMonth() + 1}&year=${currentDate.getFullYear()}`
      );
  ...
  const handleDayClick = (day) => {
    if (day && day >= new Date(new Date().setHours(0, 0, 0, 0))) {
      ...
      navigate(`/bookingStation04/timeslot/${formattedDate}`);
    }
  };
